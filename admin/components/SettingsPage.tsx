...
const SettingsPage: React.FC = () => {
  ...
  const checkConnectivity = async () => {
    ...
    // Clean URL for check
    const cleanUrl = settings.apiGatewayUrl.replace(/\/+$/, "");
    
    try {
      const res = await fetch(`${cleanUrl}/api/v1/admin/status`, {
  ...
  const handleSave = () => {
    ...
    localStorage.setItem('cf_gateway_url', cleanUrl); // Save clean URL
    localStorage.setItem('cf_ca_key', settings.caKey);
    
    // Update state to match clean URL
    setSettings(prev => ({ ...prev, apiGatewayUrl: cleanUrl }));
